<!DOCTYPE html>
<html lang="ru">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Wedkowanie</title>
    <link href="Example1.css" rel="stylesheet">
</head>

<body>
    <section id="banner">
        <h1>
            Portal dla wedkarzy
        </h1>
    </section>
    <section id="left1">
        <h3>
            Ryby zamieszkajace rzeki
        </h3>
        <ol>
            <?php
                $connect = mysqli_connect('localhost', 'root', '', 'wedkowanie');
                $request1 = "SELECT akwen, wojewodztwo FROM lowisko WHERE Ryby_id = 2";
                $wynik1 = mysqli_query($connect, $request1);
                while ($row1 = mysqli_fetch_row($wynik1))
                {
                    echo "<li>$row1[0] $row1[1]</li>";
                }
            ?>
        </ol>
    </section>
    <section id="left2">
        <h3>
            Ryby drapizne naszych wod
        </h3>
        <table>
            <tr>
                <th>L.p</th>
                <th>Gatunek</th>
                <th>Wystepowanie</th>
            </tr>
            <?php
                $request2 = "SELECT id, nazwa, wystepowanie FROM ryby where styl_zycia = 1";
                $wynik2 = mysqli_query($connect, $request2);
                while ($row2 = mysqli_fetch_row($wynik2))
                {
                    echo "<tr>
                    <td>
                        $row2[0]
                    </td>
                    <td>
                        $row2[1]
                    </td>
                    <td>
                        $row2[2]
                    </td>
                    </tr>";
                }
            ?>
        </table>
    </section>
    <section id="right">
        <img src="images.jpg" alt="Piekna ryba" width="400" height="320">
    </section>
    <section id="footer">
        <p>Strone wykonal: Aliaksei Novik, Я сосу большой пинос у негра</p>
    </section>
</body>

</html>